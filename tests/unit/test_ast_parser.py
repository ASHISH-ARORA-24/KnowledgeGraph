...
def test_parse_file_returns_list(tmp_path):
    f = tmp_path / "empty.py"
    f.write_text("")
    assert isinstance(parse_file(str(f), "team-alpha"), list)


def test_parse_file_always_returns_module_node(tmp_path):
    f = tmp_path / "mod.py"
    f.write_text("")
    nodes = parse_file(str(f), "team-alpha")
...
def test_parse_file_different_teams_produce_different_node_ids(tmp_path):
    f = tmp_path / "tax.py"
    f.write_text(FUNC_SOURCE)
    alpha_ids = {n.node_id for n in parse_file(str(f), "team-alpha")}
    beta_ids  = {n.node_id for n in parse_file(str(f), "team-beta")}
...
def test_parse_file_node_ids_are_stable_across_runs(tmp_path):
    f = tmp_path / "tax.py"
    f.write_text(FUNC_SOURCE)
    first  = [n.node_id for n in parse_file(str(f), "team-alpha")]
    second = [n.node_id for n in parse_file(str(f), "team-alpha")]
    assert first == second
